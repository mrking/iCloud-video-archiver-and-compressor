...
def test_calculate_sha256_returns_hex_string(real_file: Path) -> None:
    digest = calculate_sha256(real_file)
    assert len(digest) == 64  # SHA-256 hex = 64 chars
    assert all(c in "0123456789abcdef" for c in digest)


def test_calculate_sha256_deterministic(real_file: Path) -> None:
    d1 = calculate_sha256(real_file)
    d2 = calculate_sha256(real_file)
    assert d1 == d2


def test_calculate_sha256_zero_length_file(tmp_path: Path) -> None:
...
def test_calculate_sha256_different_content_different_hash(tmp_path: Path) -> None:
    f1 = tmp_path / "a.mp4"
...
def test_empty_file_can_be_uploaded(tmp_path: Path, s3_cfg: S3Config) -> None:
    empty = tmp_path / "empty.mp4"
    empty.write_bytes(b"")
...
def test_nonexistent_file_raises_file_not_found_error(s3_cfg: S3Config, tmp_path: Path) -> None:
